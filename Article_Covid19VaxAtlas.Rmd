...
## 3.6. Infection: GSE189039 and GSE201530
...
infection_metadata = ann_vaccines_samples %>% 
  select(sample, condition, age, sex, gse_id, country, enroll_time, sequencer) %>% 
  right_join(colnames(infection_counts_ready) %>% as.data.frame() %>% rename(sample = "."))
