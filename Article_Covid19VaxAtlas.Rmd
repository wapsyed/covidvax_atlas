...
# 1. Select studies and retrieve data
...
## Batch effects and normalization (RESOLVER)

```{r}
###### Batch effects ----
# BiocManager::install("ExpressionNormalizationWorkflow")
library(ExpressionNormalizationWorkflow)
conflicted::conflicts_prefer(ExpressionNormalizationWorkflow)
...
pvca_df = pvca$dat %>% 
  ...
  mutate(batch = as.factor(batch),
         ...
         batch = fct_reorder(batch, wavgpv)) %>% 
...
ggsave(pvca_df_plot, file = here("Figures", "covid_vax_pvca.png"))
...
pca_res <- prcomp(pvca_df, scale. = TRUE)
pvca_df_plot = autoplot(pca_res, data = iris, colour = 'Species')
...
a = samples_by_condition %>% 
  ...
  ylim(0, 80)
a
ggsave(a, file = "demographics_samples-by-condition.png", width = 20, height = 10)
...
```{r warning=FALSE}
degs_annotated_mainGOs_stats <- read_csv("degs_annotated_mainGOs_stats.csv")
...
patch
ggsave(patch, width = 15, height = 10, file = "MainGOs_percgenes_boxplot_up_down.png")
```
...
shared_genes_infection_vaccination = shared_genes_infection_vaccination_comparisons %>% 
  filter(comparison == "Infection and vaccination")

shared_genes_infection = shared_genes_infection_vaccination_comparisons %>% 
  filter(comparison == "Infection only")

shared_genes_vaccines = shared_genes_infection_vaccination_comparisons %>% 
...
ggsave(shared_genes_vaccines_infection_plot/shared_genes_vaccines_only_plot/shared_genes_infection_only_plot, file = "Genes_shared_Vaccination_Infection_united.png", width = 10, height = 9)
...
```{r}
# Plotting ------
shared_genes_infection_vaccination = non_Immune_shared_genes_infection_vaccination_comparisons %>%
  select(-"...1") %>% 
  filter(comparison == "Infection and vaccination")

shared_genes_infection = non_Immune_shared_genes_infection_vaccination_comparisons %>% 
  filter(comparison == "Infection only")

shared_genes_vaccines = non_Immune_shared_genes_infection_vaccination_comparisons %>% 
  filter(comparison == "Vaccination only")
...
ggsave(shared_genes_vaccines_infection_plot/shared_genes_vaccines_only_plot/shared_genes_infection_only_plot, file = "NonImmune_Genes_shared_Vaccination_Infection_united.png", width = 10, height = 9)
```
