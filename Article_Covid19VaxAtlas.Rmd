...
# Função para realizar clustering para cada valor da coluna day
...
data_annotation
#Matrix (genes, sample)
matrix = ann_vaccines_pca_matrix_ready %>% 
  t() %>% 
  as.data.frame() %>% 
...
pairsplot_disease_vac = pairsplot(p,
          components = getComponents(p, c(1:10)),
          triangle = T, 
          trianglelabSize = 12,
          hline = 0, vline = 0,
...
```{r fig.height=8, fig.width=10, cache = TRUE}
#Loadings plot
loadingsplot = plotloadings(p, labSize = 3,  shapeSizeRange = 5,
                            components = 1:10,
             col = c(low = '#4DBBD5FF', mid = "white", high = '#DC0000FF') )

#Meta variables
metavars = eigencorplot(p,
...
metadata = data_annotation %>% 
  ...
  column_to_rownames("condition") %>% 
  filter(disease_vac!= "I") %>% 
  select(type, vaccine, dose, day, week, set)
