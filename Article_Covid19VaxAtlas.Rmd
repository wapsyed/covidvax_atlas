...
demographics_time %>% 
  ...
  # facet_wrap(vars(week), scales = "free_x", nrow = 1L)
```
...
df_input <- df %>%
  ...
  mutate(fct_relevel(outcomevar, order_levels))
...
```{r fig.height=10, fig.width=10}
normalized_counts_long = normalized_counts %>% rownames_to_column("genes") %>% 
...
condition = annotations %>% select(condition)
...
table = df %>% 
  ...
  mutate(condition = factor(condition, levels = c(unique(condition[order(condition)]))),
         condition = paste0(condition, " (n=", samples, ")"),
         genes = fct_relevel(genes, genes_rf$Variable))
...
disease_vac_boxplot_samples_VIP = table_ready %>% 
  filter(genes == "GZMM") %>% 
  ggplot() +
  aes(x = condition, y = counts, fill = if_else(day == 0, "Day 0", type)) +  
  geom_hline(yintercept = 0, linetype = 2) +
  scale_y_log10()+
  geom_jitter(aes(color = if_else(day == 0, "Day 0", type)),
              alpha = 0.3, size = 0.1) +
