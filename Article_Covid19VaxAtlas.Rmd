...
Serology_data %>%
  ...
  mutate(pseudovirus_neutralisation_id50 = ifelse(pseudovirus_neutralisation_id50 <= 0, 1e-10, pseudovirus_neutralisation_id50)) %>%  # Substituir zeros ou valores negativos
  summarise(
    pseudovirus_neutralisation_id50_gmt = log10(prod(pseudovirus_neutralisation_id50)^(1/n()))  # Calcular a média geométrica
  )
...
## 3.4. Infection: GSE189039 and GSE201530
...
#Save
all_degs %>% 
  saveRDS(file = here("DGE analysis", "all_studies_degs_wald.csv"))

all_degs
```

## 3.5. Weight and adjust p values
...
add = GSE201530_metadata_annotate %>% 
              select(day, condition, gse_id, vaccine, week:sequencer) %>% 
  distinct() %>% 
  anti_join(ann_vaccines,
            by = "condition")
...
###### 4.4.2.2.1. Immune vs. non-immune genes
...
#### 10.5.1.1. Genes shared by process (immune and non immune)
```{r}
