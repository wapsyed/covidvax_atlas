...
demographics_days_samples_doses = ann_vaccines_conditions %>% 
  mutate(condition = as.factor(condition),
         ...
         condition = if_else(condition == "BNT (V1, D0)" & gse_id == "GSE189039", "H (D0)", condition)) %>%
  separate(condition, into = c("condition"), sep = " \\(") %>%
  mutate(dose_f = if_else(day_f == "0", "0", dose_f),
         condition = if_else(severity != "H", 
                             paste0(condition, " (", severity, ")"),
                             condition),
...
demographics_samples_bar = demographics_days_samples_doses %>% 
  ggplot(.) +
  aes(x = samples, ,
      y = condition, 
      fill = type,
...
demographics_plot_days_samples_doses + demographics_samples_bar +
