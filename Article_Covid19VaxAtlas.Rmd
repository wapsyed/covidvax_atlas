...
```{r }
gse189039_I_moderate_D10 = results(dds, name = "condition_BNT..V1..D10._vs_BNT..V1..D0.") %>% 
  as.data.frame() %>% 
  arrange(padj) %>% 
...
all_degs_weighted =  all_degs_annotated %>% 
  group_by(genes, condition) %>%  
  summarise(weighted_avg_fc = sum(log2FoldChange * n) / sum(n),
            .groups = 'drop') %>% 
  inner_join(all_degs_annotated, by = join_by("condition", "genes")) 

pval_weighted <- all_degs_weighted %>%
  group_by(genes, condition) %>%
  summarise(
    weighted_p = wtd.mean(pvalue, weights = n),  # Calcula a média ponderada dos p-valores usando o tamanho amostral como peso
    .groups = "drop"
  ) %>% 
  inner_join(all_degs_weighted, by = join_by("genes", "condition"))

all_degs_weighted_adjusted = pval_weighted %>% 
  mutate(fdr = p.adjust(weighted_p, method = "BH")) %>% 
  mutate(padj_tf = if_else(padj < 0.05, "sig", "ns"),
         meta_fdr_tf = if_else(fdr < 0.05, "sig", "ns"))

all_degs_weighted_adjusted %>% 
  group_by(padj_tf, meta_fdr_tf) %>% 
  summarise(n_padj_sig = n()) %>% 


ggplot(.) +
  aes(x = padj_tf, y = n_padj_sig) +
  geom_col(fill = "#112446") +
  theme_minimal() +
  facet_wrap(vars(meta_fdr_tf))
...
mutate(disease_vac = fct_relevel(disease_vac, "I", "V")) %>% 
  arrange(disease_vac) %>% 
  mutate(condition = factor(condition, levels = unique(condition[order(disease_vac)]))) %>% 
  ggplot() +
...
ggsave(patch_immune_degs, file = here("Figures", "immune_other_plot_diseaseorvac.png"), width = 15, height = 7)
ggsave(patch_immune_degs, file = here("Figures", "immune_other_plot_diseaseorvac_2.png"), width = 15, height = 10)
...
```{r}
geneset_res = all_degs_p_05_vac_infected_Immune_GO_T2G_general_GSEA_ALL_2024_01_17

clust_rows <- geneset_res %>% 
...
geneset_res_df = geneset_res %>% 
  mutate(id = factor(id, levels = clust_rows$labels[clust_rows$order]),
         condition = factor(condition, levels = clust_cols$labels[clust_cols$order])) %>% 
  inner_join(ann_vaccines_conditions, by = "condition")

dotplot = geneset_res_df %>% 
  select(condition, id, nes, qvalue) %>% 
  ggplot(.) +
  aes(x = condition, y = id, color = nes, size = qvalue) +
  geom_point() +
...
#              by = "condition") %>% 
#   column_to_rownames("condition") %>% 
#   select(disease_vac, vaccine, day, week, dose, infection, type) %>% 
#   mutate(day = as.factor(day),
#          week = as.factor(week),
#          dose = as.factor(dose),
...
#           scale = "rows",
#           cluster_cols = T,
#           annotation_cols = annotations_col,
#           annotation_color = col,
#           shape = "circle",
# ) %>% 
#   ggheatmap_theme(1,theme =list(theme(axis.text.x = element_text(angle = 45L,
#     hjust = 1L,
#     vjust = 1L))))
...
#### 5.3.6. Bubbleplot (Heatmap with ggplot)
...
df = all_degs_p_05_vac_infected_Immune_GO_specific_GSEA_ALL_2024_01_17 %>% 
  clean_names() %>% 
  mutate(logq = -log10(qvalue)) %>% 
  inner_join(ann_vaccines_conditions, by = "condition") %>% 
  mutate(day = as.factor(day),
...
## 5.4. ssGSEA for Cell deconvolution
...
### Cell deconvolution with GSEA for single sample
```{r}
CellMarker_ssGSEA <- all_samples_l2fc %>%
  select(condition = sample, genes, log2fold_change = log2fc) %>% 
  autoGSEA(TERM2GENE = CellMarker_genes, 
           geneset_name = "CellMarker PBMC") 


CellMarker_ssGSEA_1 = CellMarker_ssGSEA %>% 
  clean_names() %>% 
...
CellMarker_ssGSEA_2 = CellMarker_ssGSEA_1 %>% 
...
### Cell deconvolution with immunedeconv
...
xcell_results_annotated_2 = xcell_results_annotated %>% 
  mutate(condition = fct_relevel(condition, 
...
#boxplot
xcell_results_annotated_2_plot =  xcell_results_annotated_2%>% 
  separate(condition, into = c("condition", "n"), sep = "\\(n=")
...
all_covid_vax_atlas =  read_csv(here("DGE analysis","all_degs_p_05_vac_infected_19_12_23.csv"))
