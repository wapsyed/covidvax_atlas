...
degs_covid_vax_immune = degs_covid %>% 
  bind_rows(degs_hagan) %>% 
  inner_join(ImmuneGO_Annotated_Genes %>%
               select(genes) %>%
               distinct(), 
...
degs_covid_vax_immune %>% 
  saveRDS(file = here("VaxGO gene sets", "degs_covid_vaxhagan_immune.rds"))
...
# Função para realizar clustering por dia e disease_vac
cluster_by_day_and_disease_vac <- function(df, day_column = "day", condition_column = "condition", disease_vac_column = "disease_vac") {
  
  # Obter os níveis únicos do fator da coluna de dias
  days <- unique(df[[day_column]])
  ...
  for (day_value in days) {
    ...
    # Filtrar os dados por dia
    df_day <- df %>%
      filter(!!sym(day_column) == day_value)
    
    # Loop por cada valor de disease_vac ("v" ou "i")
    for (vac_group in unique(df_day[[disease_vac_column]])) {
      
      # Filtrar os dados para o grupo disease_vac atual
      df_vac <- df_day %>%
        filter(!!sym(disease_vac_column) == vac_group) %>%
        select(!!sym(condition_column)) %>%
        distinct()
      
      # Verificar se há mais de uma condição para o dia e o grupo vac
      if (nrow(df_vac) > 1) {
        # Criar uma matriz binária onde 1 representa a presença da condição
        presence_matrix <- table(df_vac[[condition_column]]) > 0
        dist_matrix <- as.matrix(presence_matrix)
        ...
        # Armazenar a ordem dos clusters
        orders[[paste(day_value, vac_group, sep = "_")]] <- col_clustered$labels[col_clustered$order]
      } else {
        # Se houver apenas uma condição, armazenar o rótulo
        orders[[paste(day_value, vac_group, sep = "_")]] <- as.character(df_vac[[condition_column]])
      }
    }
  }
  ...
  # Converter a lista de ordens para um data frame
  order_df <- data.frame(
    group = rep(names(orders), sapply(orders, length)),
    condition = unlist(orders)
  )
...
result <- cluster_by_day(df, day_column = "day") %>% 
  mutate(day = fct_relevel(day, df$day %>% unique() %>% sort() %>% as.character()))

column_order <- result$condition
...
xcell_results_annotated_2_heatmap 
...
xcell_results_annotated %>% 
  ...
  mutate(condition = fct_relevel(condition, c(ann_vaccines_covid_other_conditions %>%
                                                mutate(day = as.character(day),
                                                       day = as.integer(day)) %>% 
                                                arrange(disease_vac, day) %>% pull(condition)))) %>% 
  group_by(cell_type) %>% 
  ungroup() %>% 
  ...
  #                         "Monocyte")) %>% 
  ggplot() +
  aes(x = condition, y = cell_type, fill = value) +  
  geom_tile() +
  theme_few() +
  theme(axis.text.x = element_text(angle = 45,
                                   hjust = 1,
        ...
        panel.grid.minor = element_line(size = 0.05)) +
  labs(y = "",
       title = "Cell deconvolution, XCell, selected cells",
       fill = "Type") +
  guides(color = "none") +
  scale_fill_gradient2(name = "Normalized Score", 
                       limits = c(-1, 1),
                       low = "#4DBBD5FF",    
                       mid = "white",
                       oob = scales::squish,
                      high = "#DC0000FF")  +
  ggforce::facet_row(~disease_vac, scales = "free_x", space = "free")
