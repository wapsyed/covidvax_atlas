...
Download general data (counts data and metadata).

GEO supplementary files. Download individual studies. I recommend
downloading large files from the GEO website. As we couldn't retrieved all GSE199750 counts matrices through R, we downloaded the counts table from the website: <https://combine-australia.github.io/RNAseq-R/06-rnaseq-day1.html>. 
...
getGEOSuppFiles('GSE189039', makeDirectory = T)
untar('GSE189039_RAW.tar')
gunzip("GSE174621_RAW.tar") 

############ Metadata

###### GSE189039
filename = "GSE189039"
GSE189039 <- getGEO("GSE189039", GSEMatrix = TRUE) #GSEMatrix = TRUE to retrieve samples metadata
...
###### GSE201533
GSE201533 <- getGEO("GSE201533", GSEMatrix = TRUE) 
...
###### GSE201530
getGEOSuppFiles('GSE201530')
untar('GSE201530_RAW.tar')
filename = "GSE201530"
...
gse201533 <- getGEO("GSE201533", GSEMatrix = TRUE)
gse201533 <- gse201533[[1]]
gse201533_metadata <- pData(gse201533) 
colnames(gse201533_metadata)
gse201533_metadata <- select(
  gse201533_metadata,
  "title",
  "geo_accession",
  "source_name_ch1",
...
gse199750 <- getGEO("GSE199750", GSEMatrix = TRUE) 

gse199750 <- gse199750[[1]]

gse199750_sampleinfo <- pData(gse199750) %>% 
  select(
    GEO = "geo_accession", 
...
gse199750_sampleinfo <- gse199750_sampleinfo %>%
  mutate(FirstSecondDose = coalesce(FirstSecondVaccine, Vaccine)) %>% 
  select("GEO", "SampleN", "SubjectID", "Age", "Sex", "Timepoint", "FirstSecondDose", "ThirdVaccine", "Tissue", "Run") 
...
for(arquivo_gz in arquivos_gz_gse201533) {gunzip(arquivo_gz)}
...
colnames(counts_gse201533) <- paste0("Sample_", seq_along(lista_dados_gse201533))
colunas_descritivas <- lista_dados_gse201533[[1]][,1]
counts_gse201533 <- cbind(counts_gse201533, colunas_descritivas)
...
Read files and convert to tables.
...
# Read files
files_txt_GSE201530 <- list.files(path = here("GSE201530"), pattern = "*.txt$", full.names = TRUE)
files_txt_GSE189039 <- list.files(path = here("GSE189039"), pattern = "*.txt$", full.names = TRUE)
...
# Filter null elements in the list and merge dataframes
...
GSE199750: Gene annotation
Fifty-eight dataframes were generated, each with 60,671 rows and 7 columns describing each gene. However, each gene is labeled with its EnsembleID. Therefore, convert them into symbols using the 'org.Hs.eg.db' package or biomaRt. Record key information about each gene, such as biotype and Entrez ID.
...
for(i in seq_along(list_data_GSE199750)) {

  ensembl_ids <- list_data_GSE199750[[i]]$Geneid
  gene_info <- getBM(attributes = c("gene_biotype", "entrezgene_id", "hgnc_symbol"), 
                   filters = "ensembl_gene_id", 
                   values = ensembl_ids, 
                   mart = mart)
  
  list_data_GSE199750[[i]] <- merge(list_data_GSE199750[[i]], gene_info, by.x = "Geneid", by.y = "ensembl_gene_id")
}
```

GSE199750: Data cleaning
```{r eval=FALSE, include=FALSE}
total_rows <- max(sapply(list_data_GSE199750, nrow))
counts_gse199750 <- data.frame(matrix(NA, 
                                      nrow = total_rows),
                                      ncol = length(list_data_GSE199750)))
#Rename the columns (e.g., "X21.01684.hisat2.bam") with the corresponding GSM codes, which in this case start with GSM5983813. This will facilitate data integration.
colnames(counts_gse199750) <- paste0("GSM", seq_along(list_data_GSE199750) + 5983812) 

counts_gse199750 <- list_data_GSE199750 %>%
  map_dfc(~ .x[, 7]) %>%
  mutate(across(everything(), ~ replace_na(., 0)))
colunas_descritivas <- list_data_GSE199750[[1]] %>%
  select(Geneid, hgnc_symbol, gene_biotype)  
counts_gse199750 <- bind_cols(colunas_descritivas, counts_gse199750) %>%
  relocate(Geneid, hgnc_symbol, gene_biotype) 
...
The count table provided by the study is named with Subject ID + Timepoint and not with the GSM code. Therefore, we will need to process the metadata table. Additionally, it lacks the age column. These data can be manipulated in Excel.
...
hgnc_symbol_index <- which(names(raw_RNASeq_counts_ann) == "hgnc_symbol") 
...
# (TERMINAR) Principal Variance Components Analysis
...
## (TERMINAR) Gene expression by gender
...
A ontologia de processo biológico "immune system process" (<GO:0002376>)
foi selecionada para uma análise direcionada das DEGs em relação à
resposta imunológica. O dataset foi baixado pelo QuickGO filtrando os
campos de Taxon (H. sapiens), Gene Products (all marked, except for
"unreviewed") e Aspect ("Biological Process").

Para ler mais sobre códigos de evidencia:
...
# Retrieve collections from MSigDB  -------
msigdbr_collections() #List all 
...
Manual annotation Os estudos foram filtrados pelo tamanho do conjunto de
genes compartilhados (GSSize \> 10 ou \>50) e com pvalue (\<0.25). Os
dados foram anotados no R e depois avaliados manualmente no excel.

```{r}
#Importar arquivo
VaxSigDB_genesets = here("VaxGO gene sets", "VAX_GeneSets_Annotated_26_10_23")
...
#Salvar
write.csv(VAX_Genes, file = here("VaxGO gene sets", "VAX_Genes_Annotated_RAW.csv"))
write.csv(VAX_GeneSets_annotated, file = here("VaxGO gene sets", "VAX_Genesets_Annotated_RAW.csv"))

#Display
head(VAX_GeneSets_annotated)

VAXSigDB_Annotated = VAXSigDB_Annotated %>% 
  mutate(VACCINE_CONDITION = paste0(VACCINE, " (", `DATE-TIME`, ")")) %>% 
  select(SYSTEMATIC_NAME, VACCINE_CONDITION, VACCINE, everything())

VAXSigDB_Annotated %>% write.csv(file = here("VaxGO gene sets","VAXSigDB_Annotated.csv"), row.names = F)
...
ggsave(platforms_hist, file = here("VaxGO gene sets", "VAXSigDB_Platforms_hist_1.png"), width = 10, height = 5)
...
ggsave(pathogens_hist, file = here("VaxGO gene sets", "VAXSigDB_Pathogens_hist_1.png"), width = 10, height = 5)
...
# Subset de vacinas para as seguintes análises comparativas
VAX_GeneSets_Blood_PBMC_VAC = VAX_GeneSets_annotateds_Atate_RA %>%   
  filter(SAMPLE_SOURCE %in% c("PBMC", "WHOLE BLOOD"), 
         STUDY_TYPE %in% c("VACCINE"))
...
#Salvar resultados
write.csv(shared_genes_df_mirror, file = here("VaxGO gene sets", "VaxMSigDB_sharedgenes.csv"))
...
colfun = coloRamp(c(0, 100), c("white", "#10BD"))
...
heatmap_plot_all = Heatmap(mat,
                        ...
                        heatmap_legend_aa = list(direction = "horizontal"),
