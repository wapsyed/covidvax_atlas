...
immune_other_plot_diseaseorvac_I  = degs_immune %>% 
  ...
  mutate(condition = fct_relevel(condition, c(ann_vaccines_conditions_ordered$condition_grouped %>% unique() %>% as.character()))) %>%
  ggplot() +
...
immune_other_plot_diseaseorvac_V  = degs_immune %>% 
  filter(disease_vac == "Vaccination") %>% 
    mutate(condition = factor(condition, levels = c(ann_vaccines_conditions_ordered$condition_grouped %>% unique() %>% as.character()))) %>%
  ggplot() +
 aes(x = fct_rev(factor(condition)), y = total_number, fill = totals) +
 geom_col() +
 coord_flip() +
...
immune_other_plot_diseaseorvac_I  = degs_immune %>% 
  filter(disease_vac == "Infection") %>% 
    mutate(condition = factor(condition, levels = ann_vaccines_conditions_ordered$condition_grouped %>% unique())) %>%
  ggplot() +
...
xcell_results_annotated_2_plot =  xcell_results_annotated %>% 
  ...
  aes(x = condition, y = value, 
      fill = if_else(day == 0 & disease_vac != "I", "Day 0", type)
      ) +  
