...
gene_info <- getBM(attributes = c("entrezgene_id", "hgnc_symbol", "ensembl_gene_id", "start_position", "end_position"), 
                   mart = mart) 
...
dim(ann_vaccines_pca_matrix) #1197 genes
...
dim(ann_vaccines_pca_matrix_ready) #1189 genes
...
pc = "PC1"
data_2 =  PC1 %>% 
  inner_join(data_genes, by = "id")
filename_2 = paste0(filename, "_PCA_",pc, "_COVID")

######## Matrix
matrix = data_2 %>% 
  select(id, condition, nes) %>%
  pivot_wider(names_from = "condition", 
              values_from = "nes", 
              values_fn = mean) %>% 
  replace(is.na(.), 0) %>%
...
fileimageheatmap_grouped = paste0(filename, sep ="_", "Complexheatmap.png")
