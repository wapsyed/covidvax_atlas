...
infection_metadata = ann_vaccines_samples %>% 
  select(sample, condition, age, sex, gse_id, variant, country, enroll_time, sequencer) %>% 
  left_join(colnames(infection_counts_ready) %>% as.data.frame() %>% rename(sample = ".")) %>% 
  filter(sample %in% colnames(infection_counts_ready)) %>% 
  mutate(condition = if_else(condition == "BNT (V1, D0)", "H (D0)", condition),
         condition = as.factor(condition),
    condition = fct_relevel(condition, "H (D0)"),
    ...
    gse_id = as.factor(gse_id)) %>% 
  select(sample, condition, age_f, age, sex, variant)
...
infection_DEGs %>% 
  write_csv(file = here("DGE analysis", "infection_DEGs_wald.csv"))

gse199750_DEGs_wald %>% filter(condition == "ChAd (V3, D1)")
...
all_degs_p_05_vac_infected = read_rds(file = here("DGE analysis", "all_studies_degs_weighted.rds")) %>%
  filter(fdr <= 0.05) %>% 
  filter(!str_detect(condition, "\\(D0\\)$"))
