...
matrix_data_cols_rows_ordered = matrix_data %>% 
  ...
  inner_join(ann_rows_heatmap %>% rownames_to_column("vac_ref") %>% select(vac_ref), by = "vac_ref") %>% 
  arrange(vac_ref) %>%
  distinct() %>% 
...
heatmap_plot = Heatmap(matrix_data_cols_rows_ordered,
                        ...
                        col = col_fun, #color
                       cell_fun = function(j, i, x, y, width, height, fill) {
                                            if(matrix_data_cols_rows_ordered[i, j] > 0)
                                            grid.text(sprintf("%.f", matrix_data_cols_rows_ordered[i, j]), 
                                                      x, y, 
                                                      gp = gpar(fontsize = 6))},
                        row_title = "",
                        show_column_names = TRUE,
