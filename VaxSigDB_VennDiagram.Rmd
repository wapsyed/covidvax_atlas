...
```{r}
####### INPUT
data_2 = covid_other_notimmune_genes
filename_2 = paste0("NOTImmune_GO_genes_All", "_VAX_COVID")
...
covid_other_immune_genes_50_mostshared_all = stats_1 %>% 
  select(genes) %>% 
  distinct() %>% 
  inner_join(Immune_GO_genes_All, by = "genes")
```
...
```{r}
####### INPUT
data_2 = covid_other_notimmune_genes_50_mostshared_all
filename_2 = paste0("NOTImmune_GO_genes_All_50", "_VAX_COVID")
...
# Distance matrix (base R)
dist_m = shared_degs_updown_immune_SharedDEGS_Venn_fisher %>% 
  dplyr::select(Cond1, Cond2, Shared) %>% 
  replace(is.na(.), 0) %>% 
  pivot_wider(names_from = Cond2, values_from = Shared) %>% 
  column_to_rownames("Cond1") %>% 
  dist() 


# Distance matrix (factoextra)
library(factoextra)

library(esquisse)
esquisser(shared_degs_updown_immune_SharedDEGS_Venn_fisher)
...
res.dist <- get_dist(dist_m, method = "kendall")

res.dist %>% 
  as.matrix() %>% 
  as.data.frame() %>% 
  rownames_to_column("Cond1") %>% 
  pivot_longer(cols = -Cond1, names_to = "Cond2", values_to = "kendall")
...
# Network graph
dist_mi = 1/dist_m
jpeg('example_forcedraw.jpg', width=3000, height=3000, unit='px')
