...
covid_other_notimmune_genes = degs_all_updown_covid_other_notimmune %>% 
  anti_join(Immune_GO_T2G, by = "genes") %>% 
  left_join(OtherGOs_Annotated_Genes %>% select(process = term, genes) %>% filter(), by = "genes") %>% 
  distinct() %>% 
  filter(regulation %in% c("UP", "DOWN")) %>% 
  mutate(regulation_numeric = case_when(
    regulation %in% c("UP") ~ 1,
    regulation %in% c("DOWN") ~ -1,
    TRUE ~ as.numeric(regulation)),
    process = if_else(is.na(process), "NO ANNOTATION", process)) %>% 
  distinct()
...
filename_2 = paste0("OtherGOs_genes_All", "_VAX_COVID")
...
for (i in 1:length(row_order(heatmap_plot))){
if (i == 1) {
clu <- t(t(row.names(mat[row_order(heatmap_plot)[[i]],])))
gene_cluster <- cbind(clu, paste(i, sep=""))
colnames(gene_cluster) <- c("genes", "cluster")
 } else {
 clu <- t(t(row.names(mat[row_order(heatmap_plot)[[i]],])))
 clu <- cbind(clu, paste("cluster", i, sep=""))
 gene_cluster <- rbind(out, clu) %>% 
   as.data.frame()}}
