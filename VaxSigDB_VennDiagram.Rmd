...
immunego = immunego_genes

immunego = ImmuneGO_Annotated_GO_26_2_24_ %>% 
  select(-"...1") %>% 
  mutate(genes = str_split(genes, ",")) %>%
  unnest(genes)

degs_updown_p_05_vac_infected = all_degs_p_05_vac_infected_19_12_23
...
shared_genes_ann_vac = shared_genes_df_mirror %>%
  select(vaccine.y, vaccine.x, Cond2:sample_source, pmid, gene_symbols:previous_vaccination) %>%
  filter(study_type == "VACCINE", 
         sample_source == "PBMC")
...
shared_genes_ann_vac = shared_genes_df_mirror %>% 
  inner_join(ann_vaccines_19_1_24 %>% 
               select(Cond1 = condition), by = "Cond1") %>% 
  anti_join(ann_vaccines_19_1_24 %>% 
               select(Cond2 = condition), by = "Cond2")
