...
degs_all_updown_vax = VAX_Genes_Annotated_RAW %>%
  clean_names() %>% 
  mutate(condition = paste0(vaccine, " (", date_time, ")")) %>% 
  select(condition, genes = gene) %>% 
  distinct()
...
degs_all_updown_covid_other_nonimmune = degs_all_updown_covid_other %>% 
  anti_join(ImmuneGO_genes, by = "genes") %>% 
  distinct()
...
#Input
data = degs_all_updown_covid_vax_other_immune %>% 
  rename(geneset = condition) %>% 
  select(geneset, genes)

filename_1 = "degs_all_updown_covid_other_immune"
...
#Salvar resultados
write.csv(shared_genes_df_mirror, file = paste0(filename_1, "_shared.csv"))
...
#Input
data = covid_other_degs

filename_1 = "covid_other_shared_"
...
```{r}
####### INPUT
data_2 = shared_genes_df_mirror %>% clean_names()
...
matrix = data_2 %>%
  ...
  pivot_wider(names_from = "cond1", values_from = "shared_vac_only_down")
