...
# Split de treinamento em treinamento e validação
set.seed(123)  # Para reproduzibilidade
split2 <- initial_split(train_data, prop = 0.8, strata = "disease_vac")
...
# Definir receita
rf_recipe <- recipe(disease_vac ~ ., data = train_data) %>%
  step_normalize(all_predictors()) %>%
  step_smote(disease_vac, over_ratio = 1, skip = TRUE) %>%  # Sobreamostrar a classe minoritária usando SMOTE
  step_dummy(type)  # Transformar a variável type em dummy

# Definir grade de sintonia
grid <- expand.grid(
  mtry = c(1:15),
  trees = c(500, 1500, 2000, 2500)
)

# Definir controlador de treinamento
...
rf_res <- rf_wf %>%
  tune_grid(
    resamples = bootstraps(val_data, times = 20),
    grid = grid,
    control = ctrl,
...
rf_final <- rf_wf %>%
  finalize_workflow(grid) %>%
  fit(data = train_data)

# Avaliação do modelo no conjunto de teste
rf_test <- predict(rf_final, test_data) %>%
  bind_cols(test_data) %>%
  metrics(truth = disease_vac, estimate = .pred_class)
