...
df_input <- df %>%
  ...
  inner_join(ann_vaccines_samples %>% select(sample, 
                                             type), #Outcome
             by = "sample") %>%
  select(sample, 
         type, #Outcome 
...
# Create recipe
tree_rec <- recipe(type ~ ., data = trees_train) %>% #OUTCOME
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_upsample(type) #Upsample unbalanced data (OUTCOME)
...
roc_auc = regular_res %>%
  ...
  geom_point() +
  labs(y = "AUC") 
ggsave(roc_auc, file = paste("roc_auc_", outcomevar, today(), ".png"), width = 10, height = 6)
...
final_res_preds %>% 
  group_by(correct, type...10) %>% 
  tabyl(correct, type...10) %>% 
  as.data.frame() %>% 
  group_by(correct) %>% 
  pivot_longer(-correct, values_to = "value", names_to = "type") %>% 
  group_by(type) %>% 
  arrange(type) %>% 
  mutate(perc = round(value/sum(value) * 100, 0)) %>% 
  write.csv(file = paste0("accuracy", outcomevar, today(), ".csv"), row.names = F)
...
vipgenes_tidymodels_type <- read_csv("vipgenes_tidymodels_type2024-05-03.csv")
