...
```{r}
######## Anotação de GOs ########
ImmuneGO_Manual = ImmuneGO_Annotated_Genes_28_2_24 # ImmuneGOs manually annotated by Immune System, Sub-system, and cells
...
ImmuneGO_Adap_Cellular <- ImmuneGO_Adaptive %>% 
...
# Adicionar linhas
ImmuneGO_Annotated_unique <- bind_rows(new_row_adap,
                                       new_row_innate,
                                       new_row_compl,
                                       new_row_cellular,
                                       new_row_humoral,
...
TCR_BCR_repertoire_genes = TCR_BCR_repertoire %>% 
  select(genes) %>% 
  distinct()
TCR_BCR_repertoire_annotations = TCR_BCR_repertoire_genes %>% select(-genes)


ImmuneGO_Annotated_Genes_2 = ImmuneGO_Annotated_Genes %>% 
  select(genes, process:immune_tissue, go_term)


ImmuneGO_Annotated_Genes_3 = ImmuneGO_Annotated_Genes_2 %>% 
  select(process, genes) %>% 
  distinct() %>% 
  group_by(process) %>% 
  summarise(set_size = n()) %>%
  inner_join(ImmuneGO_Annotated_Genes_2, by = "process") %>% 
  filter(go_term != "NA") %>% 
  anti_join(TCR_BCR_repertoire_genes, by = "genes") %>% 
  bind_rows(TCR_BCR_repertoire %>% 
              select(process, immune_system:go_term, genes) %>% 
              mutate(go_term = "Manual") %>% 
              distinct() %>% 
              group_by(process) %>% 
              summarise(set_size = n()) %>% 
              inner_join(TCR_BCR_repertoire %>% select(process, immune_system:go_term, genes) %>% 
              mutate(go_term = "Manual") %>% 
              distinct())) %>% 
  arrange(desc(set_size)) 
  

ImmuneGO_Annotated_GO = ImmuneGO_Annotated_Genes_3 %>%
  group_by(process) %>%
  summarize(genes = paste(genes, collapse = ",")) %>% 
  left_join(ImmuneGO_Annotated_Genes_3 %>% select(-genes) %>% distinct(), by = "process") %>% 
  arrange(desc(set_size)) %>% 
  distinct() %>% 
  mutate(gene_set_short = process) %>% 
  bind_rows(ImmuneGO_Annotated_GO_4_3_24_2) %>% 
  distinct() %>% 
  mutate(process = case_when(process == "TCR REPERTOIRE" & set_size != 254 ~ "DELETE",
                             process == "TCR REPERTOIRE" & immune_sub_system == "General" ~ "DELETE",
                             process == "BCR REPERTOIRE" & set_size != 158 ~ "DELETE" ,
                             process == "BCR REPERTOIRE" & immune_sub_system == "General" ~ "DELETE",
                             TRUE ~ process)) %>% 
  filter(process != "DELETE")
...
#Salvar
write.csv(ImmuneGO_Annotated_GO, file = "ImmuneGO_Annotated_GO_5-3-24_2.csv", row.names = F)
saveRDS(ImmuneGO_Annotated_GO, file = "ImmuneGO_Annotated_GO_5-3-24_2.rds")

write.csv(ImmuneGO_Annotated_Genes, file = "ImmuneGO_Annotated_Genes_5-3-24.csv", row.names = F)
saveRDS(ImmuneGO_Annotated_Genes, file = "ImmuneGO_Annotated_Genes_5-3-24.rds")
...
Verificar se todos os genes estão no manual
```{r}
...
ImmuneGO_Annotated_Genes %>% select(process, genes) %>% anti_join(a, by = "genes")
