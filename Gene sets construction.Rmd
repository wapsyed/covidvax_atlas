...
```{r}
ImmuneGO_Annotated_unique = ImmuneGO_Annotated_Genes_28_2_24
...
ImmuneGO_Annotated_Genes_3 = ImmuneGO_Annotated_Genes_2 %>% 
  ...
  select(-set_size.x, -set_size) %>% 
  rename(set_size = set_size.y)
...
ImmuneGO_Annotated_GO = ImmuneGO_Annotated_Genes_3 %>%
  ...
  arrange(desc(set_size)) %>% 
  distinct() %>% 
  mutate(gene_set_short = if_else(is.na(gene_set_short), process, gene_set_short))
...
#Salvar
write.csv(ImmuneGO_Annotated_GO, file = "ImmuneGO_Annotated_GO_4-3-24_2.csv", row.names = F)
write.csv(ImmuneGO_Annotated_Genes, file = "ImmuneGO_Annotated_Genes_4-3-24.csv", row.names = F)
