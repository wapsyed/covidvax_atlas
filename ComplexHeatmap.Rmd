...
7. ARPP: width = unit(30, "cm"), height = unit(20, "cm") 
...
png(file = fileimageheatmap_grouped, width=width_image, height=height_image, units="cm", res=900) #Dimensões da imagem
...
Immune_GO_All = data.immunego %>% 
  ...
  merge(ann_vaccines, by = "condition", all.y=F) %>% 
  select(condition, process, enrichment_score:leading_edge, core_enrichment, vaccine:previous_vaccination) %>% 
  distinct()
