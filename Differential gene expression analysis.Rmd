...
main_go_2 = degs_GO %>% 
  ...
  mutate(n_genes_perc = round(n_genes/n_genes_total * 100, 1),
         term_ancestor = if_else(term_ancestor == "biological process involved in interspecies interaction between organisms", "interspecies interaction BP", term_ancestor)) %>% 
  inner_join(ann_vaccines_19_1_24 %>% select(condition, type, disease_vac), by = "condition") 


main_go_2_plot = ggplot(main_go_2) +
 aes(x = condition, y = n_genes_perc, fill = type) +
 geom_col() +
 coord_flip() +
 theme_minimal() +
 theme(legend.position = "top") +
 ...
 vars(term_ancestor), scales = "free") +
  ...
  geom_text(aes(label = str_replace(paste0(n_genes_perc, "% /n (", n_genes, ", ", n_genes_total, ")"), "%", "%\n"), y = n_genes_perc), 
...
ggsave(main_go_2_plot, file = "main_go_2_plot.png", width = 20, height = 20)
