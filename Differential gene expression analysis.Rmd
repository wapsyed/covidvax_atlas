...
immunego_genes = immunego_genes %>%
      mutate(immune_nonimmune = "Immune") %>%
      select(genes, immune_nonimmune) %>%
      distinct()
...
plot_degs <- degs_all_updown %>%
  ...
  mutate(immune_nonimmune = if_else(is.na(immune_nonimmune), "Other", immune_nonimmune),
         immune_nonimmune = as.factor(immune_nonimmune),
         immune_nonimmune = fct_relevel(immune_nonimmune, "Other", "Immune")) %>% 
  group_by(condition, immune_nonimmune) %>% 
  summarise(n_immune = n()) %>%
  ungroup() %>% 
  inner_join(ann_vaccines, by = "condition")
...
ggplot_degs = plot_degs %>%
 ggplot() +
  aes(x = condition, fill = immune_nonimmune, weight = n_immune) +
  ...
  theme(legend.position = "right",
        axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 7),
        axis.text.y=element_text(size = 20, color = "white", angle = 0)) +
  geom_text(aes(label = n_immune, y = n_immune), 
            position = position_dodge(width = 0.9), 
            vjust = 0.2, 
            hjust = -0.2, 
            size = 3,
...
ggsave(ggplot_degs, file = "Immune_nonimmune_degs.png",  width = 10, height = 8) 
