...
mapped_notmapped = degs_GO %>% 
  select(condition, genes, direction, mapped) %>% 
  distinct() %>% 
  group_by(condition, direction, mapped) %>% 
  summarise(n_genes = n()) %>% 
  group_by(condition, direction) %>%
  mutate(total_genes = sum(n_genes)) %>% 
  mutate(perc = round((n_genes / total_genes) * 100, 1)) 

 mapped_notmapped_plot =  ggplot(mapped_notmapped) +
  aes(x = condition, y = perc, fill = mapped) +
  geom_col() +
  scale_fill_manual(
     values = c("Not mapped" = "#e1e5f2",
               "Mapped" = "#4cc9f0")) +
  theme_minimal() +
  coord_flip() +
  ylab("Genes (%)") +
  ...
  theme(legend.position = "right",
    ...
    geom_text(aes(label = perc, y = perc), 
            position = position_dodge(width = 0), 
            vjust = 0.5, 
            hjust = -0.2, 
            size = 3,
            angle = 0) +
  ggtitle("Mapped genes")
  
ggsave(mapped_notmapped_plot, file = "mapped_notmapped_stacked_perc.png",width = 10, height = 10)
```
...
degs_go_main_go = degs_GO %>%
  ...
  mutate(total_genes = sum(n_genes)) %>% 
  mutate(perc = round((n_genes / total_genes) * 100, 1)) %>% 
  mutate(term_ancestor = if_else(term_ancestor == "biological process involved in interspecies interaction between organisms", "interspecies interaction", term_ancestor))
```


```{r}
degs_go_main_go$term_ancestor <- factor(degs_go_main_go$term_ancestor, levels = c(
  "Not mapped",
...
degs_go_main_go_plot 

ggplot(degs_go_main_go) +
  aes(x = fct_rev(condition), y = perc, fill = term_ancestor) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  ylab("Genes (%)") +
  ...
  scale_fill_manual(values = c(
    "Not mapped" = "#e1e5f2",
    "interspecies interaction" = "#4cc9f0",
    'biological regulation'	= "#7ae582",
    'cellular process'	= "#5e60ce",
    "immune system process" = "#ffbc42",
    "locomotion" = "#b388eb",
    "positive regulation of biological process" = "#219ebc",
    "regulation of biological process" = "#d9ed92",
    "reproductive process" = "#06d6a0"))


ggsave(degs_go_main_go_plot, file = "degs_go_main_go_stacked_plot.png", width = 10, height = 10)
```
