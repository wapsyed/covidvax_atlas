...
```{r}
#Metadata ------
gse189039_metadata = GSE189039_metadata_1_
gse199750_metadata = gse199750_metadata_annotated_14_11_23_2_
gse206023_metadata = `gse206023_metadata_ready (1)` %>% 
  rownames_to_column("sample_id") %>% 
  separate(sample_id, into = c("participant_id", "day"), sep = "_", remove = F) %>% 
  mutate(day = str_extract(day, "\\d+"),
         dose = 3) %>% 
  clean_names() %>% 
  select(-timepoints)

write.csv(gse206023_metadata, file = "gse206023_metadata_ready.csv")
...
a = samples_by_condition %>% 
  ...
  labs(x = "")
a
ggsave(a, file = "demographics_samples-by-condition.png", width = 8, height = 10)



#### AGE BY GSE
participants = Demographics_Participants 


esquisser(participants)
...
ggsave(age, file = "Demographics_age.png", width = 10, height = 10)
...
participants = Studies_Participants %>% 
  select(Doses, Participants, Vac_Infection) %>% 
  distinct() %>% 
  group_by(Doses) %>% 
  summarise(n_total_participants = sum(Participants)) %>% 
  inner_join(Studies_Participants, by = "Doses") %>% 
  rename(Condition = Doses) %>% 
...
p = participants %>%
 filter(!is.na(Condition)) %>% 
  ggplot(aes(x = fct_reorder(Condition, n_total_participants), 
             weight = n_total_participants, 
             fill = Vac_Infection)) +
  geom_bar() +
  scale_fill_manual(name = "Vaccine or infection", 
    values = c( "BBIBP" = "#76c893",
                "ZF2001" = "#b5e48c",
                "BNT" = "#56cfe1",
                "MO" = "#72ddf7",
...
ggsave(p, file = "demographics_participants-by-condition.png", width = 8, height = 10)
...
col_fun <- colorRamp2(c(0, 100, 1000, 2000), c("white", "#f098a7","#ef233c","#d90429"))
...
ann_vaccines_heatmap = ann_vaccines_heatmap %>% 
  select(type, infection, vaccine, dose, week, day)
...
all_degs_p_05_vac_infected_notimmune = all_degs_p_05_vac_infected_29_11_23 %>%
  anti_join(immunego %>%
              select(genes) %>%
              distinct(),
            by = "genes") %>%
...
write.csv(all_degs_p_05_vac_infected_notimmune, file = "all_degs_p_05_vac_infected_notimmune.csv", row.names = F)
...
all_degs_p_05_vac_infected_cellcycle = all_degs_p_05_vac_infected_29_11_23 %>% 
  inner_join(cellcycleGO %>% 
              select(genes) %>% 
              distinct(),
            by = "genes") %>% 
  distinct() %>% 
  mutate(bioprocess = "Cell cycle")

write.csv(all_degs_p_05_vac_infected_cellcycle, file = "all_degs_p_05_vac_infected_cellcycle.csv", row.names = F)


all_degs_processes = bind_rows(all_degs_p_05_vac_infected_immune, 
                               all_degs_p_05_vac_infected_cellcycle)


all_degs_processes_other = all_degs_p_05_vac_infected_29_11_23 %>%
  anti_join(all_degs_processes %>%
              select(genes) %>%
              distinct(),
            by = "genes") %>%
  distinct() %>% 
...
all_degs_processes = bind_rows(all_degs_processes, 
                               all_degs_processes_other)


write.csv(all_degs_processes, file = "all_degs_processes_annotated.csv", row.names = F)
...
ggsave(ggplot_degs, file = "degs_immune_cellcycle_other.png",  width = 15, height = 8) 
...
ggsave(ggplot_degs, file = "degs_immune_percent_bars.png",  width = 15, height = 8) 
...
ggsave(ggplot_degs, file = "degs_immune_percent_columns.png",  width = 10, height = 8) 
...
degs_all_updown = degs_all_updown %>% 
  distinct()
...
shared_genes_infection = degs_all_updown %>%
  ...
  select(genes, infection_vaccination,n_conditions, gene_set_short, process) %>% 
...
shared_genes_comparisons = bind_rows(shared_genes_infection, 
                                     shared_genes_vaccines, shared_genes_vaccines_infection)
...
```{r}
esquisser(shared_genes_vaccines_infection_plot)
...
shared_genes_vaccines_infection_plot = shared_genes_vaccines_infection %>% 
  ...
  ggplot() +
  aes(
    x = reorder(genes, desc(n_conditions)),
    y = n_conditions,
...
shared_genes_vaccines_only = shared_genes_vaccines %>% 
  ...
  inner_join(ImmuneGO_Annotated_Genes, by = "genes") %>% 
  select(genes, "Vaccination", gene_set_short, process) %>% 
  mutate(comparison = "Vaccination only")

shared_genes_vaccines_only_plot = shared_genes_vaccines_only %>% 
  select(genes, Vaccination, comparison) %>% 
  distinct() %>%
  slice_head(n=20) %>% 
  ggplot() +
  aes(x = reorder(genes, desc(Vaccination)), 
      fill = comparison, 
      weight = Vaccination) +
  geom_bar() +
  scale_fill_manual(
...
shared_genes_infection_only = shared_genes_infection %>% 
  ...
  inner_join(ImmuneGO_Annotated_Genes, by = "genes") %>% 
  select(genes, Infection, gene_set_short, process) %>% 
  mutate(comparison = "Infection only")

shared_genes_infection_only_plot = shared_genes_infection_only %>% 
  select(genes, Infection, comparison) %>% 
  distinct() %>%
  slice_head(n=20) %>% 
  ggplot() +
  aes(x = reorder(genes, desc(Infection)), 
      fill = comparison, 
      weight = Infection) +
  geom_bar() +
  scale_fill_manual(
...
shared_genes_vaccines_only_plot = shared_genes_vaccines %>% 
  ...
  slice_head(n=20) %>% 
  ggplot() +
  aes(x = reorder(genes, desc(n_conditions)), 
      fill = comparison, 
      weight = n_conditions) +
  geom_bar() +
  scale_fill_manual(
