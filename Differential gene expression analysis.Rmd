...
ann_vaccines_samples_participants = ann_vaccines_samples


participants_gse = ann_vaccines_samples_participants %>% 
  group_by(gse_id) %>% 
  summarise(participants_gse = n())
```
...
0.9, size = 2, stroke = 0.1, na.rm = TRUE)) +
...
Studies_Participants %>% 
  clean_names() %>% 
  select(doses, regimen, participants, abrev) %>% 
  mutate(n_total = sum(participants),
         comparison = "participants",
         doses = fct_reorder(doses, desc(participants))) %>% 
  ggplot() +
  aes(x = comparison, y = participants, fill = doses, group = doses) +
...
### add labels ----
participants +
  geom_text(aes(x = comparison, y = participants, label = paste0(doses, " (", participants, ")")),
            hjust = 0,
            nudge_x = 0.5
            ) +
  geom_text_repel(
    aes(color = group, label = doses ),
    # family = "Lato",
    # fontface = "bold",
    size = 8,
    direction = "y",
...
degs_GO = all_degs_processes_immune_nonimmune %>% 
  filter(direction != "NEUTRAL") %>% 
...
write.csv(degs_GO, file = "degs_annotated_allGOs.csv", row.names = F) #Save

#Mapped and not mapped DEGs
mapped_notmapped = degs_GO %>% 
  select(condition, genes, mapped) %>% 
  distinct() %>% 
  tabyl(condition, mapped) %>% 
  adorn_percentages() %>% 
  as.data.frame() %>% 
  pivot_longer(-condition, names_to = "mapped", values_to = "count") %>% 
  mutate(count = round(as.numeric(count)*100, 1),
         mapped = fct_relevel(mapped, "Not mapped", "Mapped"))

ggplot(mapped_notmapped ) +
  aes(x = condition, y = count, fill = mapped) +
  geom_col() +
  scale_fill_manual(
     values = c("Not mapped" = "#f72585",
               "Mapped" = "#5e60ce")) +
  coord_flip() +
  theme_minimal()
...
main_go_2 = degs_GO %>% 
  ...
  filter(str_detect(term_ancestor, "cellular process|biological process involved in interspecies interaction between organisms|regulation of biological process|immune system process|No annotation"),
  ...
  mutate(n_genes_perc = round(n_genes/n_genes_total * 100, 1),
         term_ancestor = case_when(
           ...
           TRUE ~ term_ancestor),
         term_ancestor = fct_relevel(term_ancestor, "No annotation", after = Inf)) %>% 
...
main_go_2_plot = ggplot(main_go_2) +
 ...
 facet_grid(vars(direction),  vars(term_ancestor)) +
  ...
  geom_text(aes(label = paste0(n_genes_perc, "% (", n_genes, ")"), 
            ...
            size = 3,
            angle = 0) +
  ggtitle("DEGs by Biological Process") +
  labs(y = "Number of genes (%)") +
  ylim(0,100)


ggsave(main_go_2_plot, file = "main_go_2_plot_values.png", width = 15, height = 15)


main_go_2_plot_novalues = ggplot(main_go_2) +
 aes(x = forcats::fct_rev(factor(condition)), y = n_genes_perc, fill = type) +
 geom_col() +
 coord_flip() +
 theme_minimal() +
 ...
 vars(term_ancestor), scales = "free") +
   scale_fill_manual(
    values = c('IN'= "#76c893", #1st Gen  vaccines
           'VV' = "#5e60ce", #3rd Gen vaccines
           'RNA' = "#56cfe1",
...
ggsave(main_go_2_plot_novalues, file = "main_go_2_plot.png", width = 20, height = 15)
...
main_go_2_plot_totals_values = main_go_2 %>%
 ...
 theme_minimal() +
 theme(legend.position = "top") +
 facet_grid(condition~direction, scales = "free") +
   scale_fill_manual(
    values = c('IN'= "#76c893", #1st Gen  vaccines
           'VV' = "#5e60ce", #3rd Gen vaccines
           'RNA' = "#56cfe1",
...
ggsave(main_go_2_plot_totals_values, file = "main_go_2_plot_totals_values.png", width = 5, height = 15)
