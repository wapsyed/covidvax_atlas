...
ann_vaccines_samples_4_12_23 = ann_vaccines_samples_4_12_23 %>% 
  mutate(week = round(day/7),0)


vip_genes_boxplot_week = df %>% 
  rownames_to_column("genes") %>% 
  pivot_longer(cols = -genes,
               names_to = "sample",
               values_to = "counts") %>% 
  inner_join(genes_rf %>% slice_max(n = 5, importance), by = "genes") %>% 
...
vip_genes_boxplot_week


df %>% esquisser()

vip_genes_boxplot_type = df %>% 
  rownames_to_column("genes") %>% 
  pivot_longer(cols = -genes,
               names_to = "sample",
               values_to = "counts") %>% 
