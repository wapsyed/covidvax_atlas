...
df =  degs_annotated_mainGOs_stats %>% 
  ...
  filter(term_ancestor != "Not mapped") %>% 
  mutate(week = as.factor(week),
...
up = df %>% 
  ...
  geom_boxplot(aes(fill = disease_vac),
               outlier.shape = NA,
               alpha = 0.3) +
  geom_point(aes(colour = disease_vac,
                  shape = type,
                 size = n_genes),
              na.rm = T) +
  theme_minimal() +
  ...
  scale_fill_npg() + 
  scale_colour_npg() +
  scale_size_continuous(range=c(1,3), breaks=c(10,500,1000))
...
patch = up/down
patch
ggsave(patch, width = 20, height = 10, file = "boxplot_up_down.png")
