...
#Input
data = degs_all_updown_covid_other %>% 
  rename(gene_set = condition)

filename_1 = "covid_other_vax_degs_ALL_"
...
############### VENN DIAGRAM
#Input
data_full = VAX_Genes_Annotated_RAW %>%
  as.data.frame() %>% 
  mutate(VACCINE_REG_TIME = paste0(VACCINE,  "_", `DATE-TIME`, "_", REGULATION))

data_target = data_full %>% 
  select(geneset = VACCINE_REG_TIME, genes = GENE) 
data_ref = ImmuneGO_Annotated_Genes_Nested_Interesting_12_12_23 %>% 
  select(geneset = process, genes)


data = bind_rows(data_target, data_ref)
...
#Salvar resultados
write.csv(shared_genes_df_mirror, file = "VAXSigDB_ORA_ImmuneGO.csv") #Alterar


# Anotar e organizar
shared_genes_df_mirror_ann = shared_genes_df_mirror %>% 
  filter(grepl("COVID-19", Cond1) & !grepl("COVID-19", Cond2)) %>% 
  merge(VAX_GO_Annotated, by.x = "Cond2", by.y = "systematic_name", all.y=F) %>% 
  merge(vax_degs_ann, by.x = "Cond1", by.y = "condition_direction", all.y = F) %>% 
  select(vaccine.y, vaccine.x, everything())
  
#Salvar
write.csv(shared_genes_df_mirror_ann, file = "vax_degs_up_VAXSigDB_sharedgenes_ann.csv")

#Reorganizar
shared_genes_df_mirror_ann = shared_genes_df_mirror_ann %>% 
  select(condition, vaccine.y, vaccine.x, Cond2:sample_source, pmid, gene_symbols:participants) %>% 
  mutate(vac_ref = paste0(vaccine.x, " (", date_time, ")")) %>% 
  rename(vaccine_condition = vaccine.y,
         vaccine_reference = vaccine.x) %>% 
  select(vac_condition = condition, vac_ref, 
         vaccine_condition, 
         vaccine_reference, 
         everything()) %>% 
  filter(vaccine)
...
VAX_Genes_Annotated = VAX_Genes_Annotated_RAW %>% 
  clean_names() %>% 
  select(gene:set_size) %>% 
  mutate(time = if_else(time == "ANY", "0", time),
         time = as.numeric(time),
         time = round(time, 0),
         time = if_else(date == "H", 1, time),
         date = if_else(date == "H", "D", date))

VAX_GO_Annotated = VAX_Genes_Annotated %>% 
  select(-gene) %>% 
  distinct()


glimpse(VAX_Genes_Annotated_RAW)

vax_genesets = VAX_Genes_Annotated_RAW  %>% 
  clean_names() %>% 
  filter(study_subtype == "VAC ONLY") %>% 
  select(gene_set = systematic_name, vaccine, genes = gene)

covid_degs = all_degs_p_05_vac_infected_29_11_23 %>% 
  as.data.frame() %>% 
...
covid_other_degs = bind_rows(vax_genesets, covid_degs)


vax_degs_up = vax_degs %>% 
  filter(direction == "UP") %>% 
  select(gene_set = condition_direction, genes)

vax_degs_down = vax_degs %>% 
...
#Input
data = vax_genesets %>%
  bind_rows(vax_degs_down)

filename_1 = "vax_degs_DOWN_vs_"
...
# Função para calcular a sobreposição entre pares de vacinas e a porcentagem de genes compartilhados
overlap_genes <- function(cond1, cond2, data) {
  genes_cond1 <- data$genes[data$gene_set == cond1] #Trocar coluna
  genes_cond2 <- data$genes[data$gene_set == cond2] #Trocar coluna
  genes_shared <- intersect(genes_cond1, genes_cond2)
  genes_notshared_cond1 <- setdiff(genes_cond1, genes_cond2)
  genes_notshared_cond2 <- setdiff(genes_cond2, genes_cond1)
...
# Obtenha uma lista de todas as vacinas únicas
unique_cond <- unique(data$gene_set)  #trocar "study" pelo nome da coluna de interesse
...
shared_genes_df$qvalue <- qvalue(shared_genes_df$pvalue)$qvalues
...
#Salvar resultados
write.csv(shared_genes_df_mirror, file = paste0(filename,"_VAXSigDB_sharedgenes.csv")) #Alterar
...
shared_genes_ann_vac = shared_genes_df_mirror_ann %>%
  select(vac_condition, vaccine_condition, vac_ref, Cond2:sample_source, pmid, gene_symbols:previous_vaccination) %>%
...
data_2 = shared_genes_df_mirror %>% 
  clean_names()
filename_2 = "shared_genes_ann_vac_allDEGS"
...
matrix = data_2 %>% 
  ...
  pivot_wider(names_from = "cond1", 
              values_from = "sharedvac_only_down
filename_2 = "vac_only_down"


######## Matrix
matrix = data_2 %>% 
  filter(qvalue <=0.05) %>% 
  select(vac_condition, vac_ref, Percentage_Shared_Cond1) %>% #Percentage_Shared_Cond1
  pivot_wider(names_from = "vac_condition", 
              values_from = "Percentage_Shared_Cond1",  #Percentage_Shared_Cond1
              values_fn = mean) %>% 
  replace(is.na(.), 0) %>% 
  column_to_rownames(var="cond2vac_ref") %>% 
  as.matrix() %>% 
  round(0)
...
ann_cols_heatmap = matrix %>% 
  ...
  arrange(condition) %>% #Ordenar linhas para visualização
  select(condition, vaccine, type, regimen, dose, week, day_total) %>% #ordenar colunas para anotação
  distinct() %>% 
  mutate_all(as.factor)
...
matrix_data = matrix %>% 
  ...
  inner_join(ann_cols_heatmap, by= "condition") %>% 
  arrange(condition) %>% 
  select(!c(vaccine:day_totalndition = ann_vaccines_6_12_23 %>% 
  select(-"...1")
ann_vaccines_reference = data_2 %>% 
  select(vac_ref, platform, target_pathogen_disease, microbe_type, date, time, age_category) %>% 
  distinct()
...
ann_cols_heatmap = matrix %>% 
  ...
  merge(ann_vaccines_condition, by.x = "vac_condition", by.y = "condition", all.y = F) %>% 
  arrange(type, dose, day) %>% #Ordenar linhas para visualização
  select(vac_condition, vaccine, type, regimen, dose, day, variant, severity) %>% #ordenar colunas para anotação
  distinct() %>% 
...
matrix_data = matrix %>% 
  ...
  as.data.frame() %>% 
  rownames_to_column(var= "condition") %>% 
  merge(ann_vaccines_condition, by.y = "condition", all = F) %>% 
  arrange(dose) %>% 
  select(!c(vaccine:previous_vaccination)) %>% 
  column_to_rownames("condition") %>% 
  t() %>% 
  as.data.frame()%>%  
  mutate_if(is.character, as.numeric) %>%
...
ann_rows_heatmap = matrix_data %>%  
  ...
  select(vaccine:infection) %>% 
  select(microbe_type, target_pathogen_disease, type, week)

matrix_data_cols_rows_ordered = matrix_data %>% 
  as.data.frame() %>% 
  rownames_to_column(var= "condition") %>% 
  arrange(condition) %>%
  inner_join(ann_vaccines_other, by = "condition") %>% 
  select(!vaccine:infection) %>% 
  column_to_rownames("conditionvac_ref") %>% 
  select(vac_ref) %>% 
  distinct() %>% 
  merge(ann_vaccines_reference, by = "vac_ref") %>% 
  select(-age_category) %>% 
  mutate_all(as.factor) %>% 
  mutate(
    time = if_else(time == "ANY", "0", time),
    ...
    day_month = case_when(
      ...
      TRUE ~ day_total),
    day_month = round(day_month, 0)) %>% 
  arrange(microbe_type, target_pathogen_disease, platform, vac_ref, day_month, time) %>%
  distinct() %>% 
  column_to_rownames(var= "vac_ref") %>% 
  select(platform, microbe_type, target_pathogen_disease)

matrix_data_cols_rows_ordered = matrix_data %>% 
  as.data.frame() %>% 
  rownames_to_column(var= "vac_ref") %>% 
  merge(ann_vaccines_reference, by = "vac_ref") %>% 
  mutate(time = if_else(time == "ANY", "0", time),
         time = as.numeric(time),
         time = round(time, 0),
         date = factor(date, levels = c("H", "D", "W", "M", "Y"))) %>% 
  arrange(microbe_type, target_pathogen_disease, platform, vac_ref, date, time) %>%
  select(!platform:age_category) %>% 
  distinct() %>% 
  column_to_rownames("vac_ref") %>% 
...
dim(matrix)
dim(matrix_data)
dim(matrix_data_cols_rows_ordered)
...
# Values colors
col_fun <- colorRamp2(c(0, 100), c("white", "#3f37c9"))
```
...
mat = matrix_data_cols_rows_ordered
width_image = 40
height_image = 30
width = unit(15, "cm")
height = unit(15, "cm")
...
file = paste0("VAXSig_SharedGenes_Shared", filename_1, filename_2)
...
file = paste0("VAXSig_SharedGenes_perc_", filename_1, filename_2)

fileimageheatmap_grouped = paste0(file, sep ="_", "Complexheatmap.png")

png(file = fileimageheatmap_grouped, width = 50, height = 30, units="cm", res=900)

heatmap_plot = Heatmap(matrix_data_cols_rows_ordered,
                        top_annotation = ha,
                        left_annotation = row_ha,
                        show_row_names = TRUE,
                        row_names_side = "left",
                        ...
                        rect_gp = gpar(col = "gray90", lwd = 0),
                        cluster_rows = FALSE,
                        row_names_gp = row_names_gpgpar(fontsize = 8),
                        rect_gp = gpar(col = "gray90", lwd = 1),
                        cluster_rows = FALSE,
                        row_names_gp = gpar(fontsize = 8), #Grande = 6, Pequeno 15
                        row_dend_width = unit(5, "cm"), #Altura do dendrograma
                        column_split = NULL, #Split column clusters 
                        row_split = NULL, #Split row clusters
                        row_gap = unit(2, "mm"),
                        ...
                        border = TRUE,
                        column_dend_reorder = TRUE,
                        width = widthunit(15, "cm"), # 20 para pequeno e grande
                        height = heightunit(15, "cm") # 20 para pequeno, 60 para grande
)
...
png(file = fileimageheatmap_grouped, width=width_image, height=height_image50, height=30,units="cm",res=800)
...
heatmap_plot = Heatmap(matrix_data_cols_rows_ordered,
                        ...
                        show_column_names = TRUE,
                        cell_fun = function(j, i, x, y, width, height, fill) {
                                            if(matrix_data_cols_rows_ordered[i, j] > 1= 50)
                                            grid.text(sprintf("%.1f", matrix_data_cols_rows_ordered[i, j]), 
                                                      x, y, 
                                                      gp = gpar(fontsize = 5))},
                        column_title = fileimageheatmap_grouped,
...
# NO value labels
fileimageheatmap_grouped = paste0(file, sep ="_", "Complexheatmap_ROW_CLUSTERED.png")

png(file = fileimageheatmap_grouped, width = width_image, height = height_image, units="cm", res=900)

heatmap_plot = Heatmap(matunit(15, "cm"), # 20 para pequeno e grande
                        height = unit(15, "cm") # 20 para pequeno, 60 para grande
)
...
# With values
fileimageheatmap_grouped = paste0(file, sep ="_", "Complexheatmap_ROW_CLUSTERED_VALUES.png")
...
heatmap_plot = Heatmap(matrix_data_cols_rows_ordered,
                        ...
                        show_column_names = TRUE,
                        cell_fun = function(j, i, x, y, width, height, fill) {
                                            if(matrix_data_cols_rows_ordered[i, j] >= 150)
                                            grid.text(sprintf("%.1f", matrix_data_cols_rows_ordered[i, j]), 
                                                      x, y, 
                                                      gp = gpar(fontsize = 5))},
                        column_title = fileimageheatmap_grouped,
...
fileimageheatmap_grouped = paste0(file, sep ="_", "Complexheatmap_ALL_CLUSTERED.png")

png(file = fileimageheatmap_grouped, width = width_image, height = height_image50, height = 30, units="cm", res=900)
...
# With values
fileimageheatmap_grouped = paste0(file, sep ="_", "Complexheatmap_ALL_CLUSTERED_VALUES.png")
...
degs_all_updown_covid = degs_updown_ %>% 
  as.data.frame() %>% 
  select(condition:direction_wide) %>% 
  inner_join(ann_vaccines, by = "condition") %>% 
  inner_join(ImmuneGO_genes) %>% 
...
ann_vaccines_covid_other_2 = data_2 %>% 
  inner_join(ann_vaccines_covid_other, by = "condition") %>% 
  select(condition, covid_other, microbe_type, target_pathogen_disease, disease_vac, type, week) %>% 
  distinct()
...
#Dose
covid_other = ann_cols_heatmap$covid_other
dend_covid_other = cluster_within_group(matrix_data, covid_other)
...
heatmap_plot <- Heatmap(
                          ...
                          row_split = NULL,
                          row_gap = unit(2, "mm"),
                          cluster_columns = dend_covid_other,
                          column_gap = unit(8, "mm"),
                          show_column_dend = FALSE,
                          show_row_dend = FALSE,
                          row_dend_side = "left",
...
ann_vaccines_covid_other = COVID_VAX_Annotation
...
degs_all_updown_covid = degs_all_updown %>% 
  as.data.frame() %>% 
