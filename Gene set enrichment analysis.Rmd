...
ora_top10 = go %>%
  group_by(condition) %>% 
  arrange(qvalue, desc(log_fdr)) %>% 
  slice_head(n = 20) %>% 
  ungroup() %>% 
  arrange(desc(log_fdr))

lollipop_plot <- ggplot(ora_top10, aes(x = description, y = log_fdr, color = log_fdr)) +
  geom_segment(aes(xend = description, yend = 0), color = "gray") +
  geom_point(size = 3) +
  coord_flip() +
  theme_minimal() +
  facet_wrap(vars(condition), scales = "free", ncol = 4L) +
  theme(legend.position = "right",
        axis.text.x = element_text(size = 5, color = "black", vjust = 1, hjust = 1),
        axis.text.y = element_text(size = 5, color = "black", vjust = 1, hjust = 1),
...
ggsave(lollipop_plot, file = "degs_up_down_notimmune_ora_top10.png", width = 20, height = 20)
