...
CellMarker_ImmuneCells #Gene sets
ImmuneGO_Annotated_Genes_Nested_Interesting_12_12_23 #Gene sets
...
geneset = ImmuneGO_Annotated_Genes_Nested_Interesting_12_12_23
ssgsea_gene = gse201533_counts_ready #Criar variável
...
nesmat.result = nesmat %>% 
  ...
  merge(metadata, by="sample", all.x = T, all.y = F) %>% #Unir com metadata
  mutate(study = filename) %>% 
  merge(geneset, by=  "process", all.x = T, all.y = F) %>% 
  select(-"...1") %>% 
  select(-genes) %>% 
  distinct()
...
ssgsea_results_unified = bind_rows(GSE201533_ImmuneGO_ssgsea_result,
          GSE199750_ImmuneGO_ssgsea_result,
          GSE201530_ImmuneGO_ssgsea_result,
          GSE189039_ImmuneGO_ssgsea_result) %>% 
  select(-"...1")
...
ssgsea_interesting.unified = ssgsea_results_unified %>% 
  filter(!condition %in% c("BNT-MO (V1, D0)", "BNT-MO (V2, D0)", "NA")) %>% 
  mutate(day = as.factor(day),
         dose = as.factor(dose),
         regimen = factor(regimen, levels = c("H", "HO", "HE","I", "I-I", "V-I", "V-I-V", "I-V-I")))
...
data = ssgsea_interesting.unified %>% 
  filter(!vaccine == "NA")
...
filename.unified = "ssgsea_results_unified_ADAPTIVE_Regimen"

# Filtrar os dados antes de passar para ggplot
filtered_data <- data %>%
  group_by(process, vaccine, condition) %>%
  summarise(n = n()) %>%
  filter(n < 5) %>%
  pull(condition)


NES_plot_unified_adaptive_regimen = ggplot(data) +
  aes(x = condition, y = nes, fill = vaccine) +
  geom_boxplot(outlier.alpha = 1, width = 0.5) +
  geom_violin(alpha = 0.1)+
  geom_point(data = subset(data, day %in% filtered_data),
             aes(x = condition, y = nes, color = vaccine),
             position = position_jitter(width = 0.2), alpha = 1) +
  scale_fill_manual(values = c("BNT" = "#56cfe1",
                                "ChAd" = "#5e60ce",
                                "MO" = "#D7B0EE",
                                "I" = "#ff4d6d",
                                "H" = "grey95")) +
  scale_color_manual(values = c("BNT" = "#56cfe1",
                                "ChAd" = "#5e60ce",
                                "MO" = "#D7B0EE",
                                "I" = "#ff4d6d",
                                "H" = "grey95")) +
  labs(
    title = "ssGSEA - Adaptive Immune System",
    subtitle = "Adaptive ImmuneGO, by dose",
    caption = "Vaccine",
    fill = "Vaccines") + 
  theme_minimal() +
  facet_wrap(vars(regimen), scales = "free_x", nrow = 1) +
  theme(plot.title = element_text(size = 20L, face = "bold", hjust = 0.5),
        axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 7),
...
#Salvar
ggsave(NES_plot_unified_adaptive_regimen, file = paste0(filename.unified, "_NES_qvalue010.png"), width = 20, height = 6)

print(NES_plot_unified_adaptive_regimen)
...
filename.unified = "ssgsea_results_unified_INNATE_Regimen"
...
#Salvar
ggsave(NES_plot_unified, file = paste0(filename.unified, "_NES_qvalue010.png"), width = 20, height = 6)

print(NES_plot_unified)
...
ssgsea_gene = gse_counts_all_long %>%  #Criar variável
  select(-'...1', -study) %>% 
  pivot_wider(names_from = "condition", values_from = "counts") %>% 
  column_to_rownames("genes")

metadata = ann_vaccines_samples_4_12_23
filename = "ssgsea_vac_infected"
...
filename = "CellMarker_ssgsea_results_unified.all"
...
NES_plot = ggplot(ssgsea_interesting) +
  ...
  scale_fill_hue(direction = 1) +
  theme_minimal() +
  facet_grid(vars(cell_name), vars(vaccine), scales = "free_x") +
 labs(title = paste0(filename, "_ImmuneGO ssGSEA"), 
      fill = "vaccine") +
 theme_bw() +
 theme(plot.title = element_text(size = 20L, face = "bold", hjust = 0.5),
...
all_degs_p_05_vac_infected_GSEA_ALL <- GSEA_ALL(all_degs_p_05_vac_infected_29_11_23, Immune_GO_T2G, CellMarker_genes, vax_sig_genes)
